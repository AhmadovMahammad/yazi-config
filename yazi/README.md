...
This config lives in the [`my-configs`](../README.md) repo. Copy the snapshot into place:

```sh
git clone https://github.com/AhmadovMahammad/my-configs.git
cp -r my-configs/yazi/. ~/.config/yazi/
ya pkg install   # restores plugins + the Flexoki Dark flavor from package.toml
```
